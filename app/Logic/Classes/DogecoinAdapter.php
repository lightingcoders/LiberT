...
class DogecoinAdapter
{
        ...
        if (is_array($output)) {
            collect($output)->pluck('amount', 'address')
                ->each(function ($amount, $address) {
                    DogecoinAddress::where('address', $address)
                        ->increment('balance', $amount);
                });
        } else {
            DogecoinAddress::where('address', $output)
                ->increment('balance', $amount);
        }
    }
    ...
    private function getWebhookUrl()
    {
        ...
        $webhook = route('blockcypher.hook.doge');
