...
class TradesController extends Controller
{
    ...
    public function complete(Request $request, $token)
    {
        if ($request->ajax()) {
            ...
            if ($trade = $trades->first()) {
                ...
                $tx = $this->processTransaction($trade);
                ...
                if (isset($tx['hash'])) {
                    ...
                    $message = __('Oops! Something went wrong... Please contact our support to resolve this.');

                    return response($message, 419);
                }
     ...
     * @return array
     */
    public function processTransaction($trade)
    {
        // Get seller's UTXOs has inputs for the transaction
        $addresses = $trade->seller()
            ->getAddressModel($trade->coin)
            ->latest()->where('balance', '>', 0)
            ->get();

        // Get adapter for the coin in context
        $adapter = getBlockchainAdapter($trade->coin);

        // Relay transaction on the network and return its data
        return $adapter->sendMultiple(
            $addresses, $trade->getOutputs()
        );
    }
    ...
    public function dispute(Request $request, $token)
    {
        if ($request->ajax()) {
            $trades = Trade::where('token', $token)
                ...
                ->filter(function ($trade) {
                    return $trade->party(Auth::user(), 'buyer') || $trade->party(Auth::user(), 'seller');
                });

            if ($trade = $trades->first()) {
                $trade->status = 'dispute';
                $trade->dispute_by = Auth::user()->name;
                $trade->dispute_comment = $request->prompt;
                $trade->save();
