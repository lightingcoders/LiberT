...
use App\Logic\Classes\DogecoinAdapter;
use App\Models\DogecoinAddress;
...
class ProcessDogecoin implements ShouldQueue
{
     ...
     */
    public function handle(DogecoinAdapter $adapter)
    {
    ...
    {
        $transactions = DogecoinTransaction::where(
            'hash', $data->get('hash')
        )->get();
    ...
    {
        $dogecoin_address = DogecoinAddress::where(
            'address', $address
        )->first();
        ...
        if($dogecoin_address){
            ...
            $dogecoin_address->update($balance->only([
