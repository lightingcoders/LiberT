...
class ProcessBitcoin implements ShouldQueue
{
    ...
    {
        $transactions = BitcoinTransaction::where(
            'hash', $data->get('hash')
        )->get();
    ...
    {
        $bitcoin_address = BitcoinAddress::where(
            'address', $address
        )->first();
        ...
        if($bitcoin_address){
            ...
            $bitcoin_address->update($balance->only([
