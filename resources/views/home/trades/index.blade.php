...
@php $title = __(':coin Trade With :payment_method', ['coin' => get_coin($trade->coin), 'payment_method' => $trade->payment_method])@endphp
            ...
            <div class="chats">
                ...
                <div class="row text-left">
                    <div class="col-xl-6">
                        ...
                        <div class="row  text-center">
                            <div class="col border-blue-grey border-2 border-lighten-3 m-1 p-1">
                                <h4 class="card-title blue-grey text-bold-500">{{__('RATE')}}</h4>
                                <p class="card-text">{{$trade->rate()}}</p>
                            </div>
                            <div class="col border-blue-grey border-2 border-lighten-3 m-1 p-1">
                                <h4 class="card-title blue-grey text-bold-500">{{__('VALUE')}}</h4>
                                <p class="card-text">{{$trade->coinValue() . strtoupper($trade->coin)}} </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-xl-6  pt-1">
                        ...
                        <div class="alert alert-light mb-2" role="alert">
                            @if($trade->type == 'buy')
                                <h4 class="alert-heading mb-1"><b>{{__('Seller Instruction!')}}</b></h4>
                            @else
                                <h4 class="alert-heading mb-1"><b>{{__('Buyer Instruction!')}}</b></h4>
                            @endif
                        ...
                        <div v-if="status === 'active'">
                            <div class="bs-callout-warning callout-border-left p-1 mb-1" role="alert">
                                <h4 class="alert-heading mb-1"><b>{{__('Important!')}}</b></h4>
                                <span>
                                    {{__('The above value of :coin is held safely by our escrow service.', ['coin' => get_coin($trade->coin)])}}

                                    @if($trade->party(Auth::user(), 'buyer'))
                                        {{__('As soon as you make the payment, you should click')}}
                                        <b>{{__('CONFIRM PAYMENT')}}</b>
                                        {{__('button to stop the counter.')}}
                                        {{__('After making payment and you presume this as a scam attempt, you should click the')}}
                                        <b>{{__('RAISE DISPUTE')}}</b>
                                        {{__('button to call the attention of a moderator.')}}
                                    @elseif($trade->party(Auth::user(), 'seller'))
                                        {{__('As soon as you see the payment, you should click')}}
                                        <b>{{__('RELEASE COIN')}}</b>
                                        {{__('button to complete the trade.')}}
                                        {{__('If you do not receive payment and you presume this as a scam attempt, you should click the')}}
                                        <b>{{__('RAISE DISPUTE')}}</b>
                                        {{__('button to call the attention of a moderator.')}}
                                    @endif
                        ...
                        <div v-else-if="status === 'dispute'">
                            <div class="bs-callout-danger text-left callout-border-left p-1 mb-1" role="alert">
                                @if(!$trade->party(Auth::user(), 'moderator'))
                                    <h4 class="alert-heading mb-1"> {{__('Attention!')}} </h4>

                                    {{__('A dispute has been raised over this trade, which one of our available moderators will attend to shortly!')}}
                                    {{__('Investigation will be conducted based on the previous chats as well as uploaded proof of payment.')}}
                                    {{__('Any decision taking by our moderator should be considered as final.')}}
                                @else
                        ...
                        @if($trade->party(Auth::user(), ['buyer', 'seller']))
                            <div v-else-if="status === 'successful'">
                                <div class="bs-callout-success text-left callout-border-left p-1 mb-1" role="alert">
                                    <h4 class="alert-heading mb-1"> {{__('Successful!')}} </h4>

                                    @if($trade->party(Auth::user(), 'buyer'))
                                        {{__('Seller has released the coin into your wallet address!')}}
                                        {{__('You may need to wait for its confirmation before it reflects on your balance.')}}
                                        {{__('To verify this, go to your wallet and check under the transaction tab')}}
                                    @elseif($trade->party(Auth::user(), 'seller'))
                                        {{__('The coin value of this transaction has been released into the wallet of the buyer.')}}
                                        {{__('The standard fee of :fee% has been charged as well for this trade.', ['fee' => $trade->fee])}}
                                    @endif
                                </div>
                            </div>
                        @endif
                                    ...
                                    <span v-if="parseInt(confirmed)">
                                        @if($trade->party(Auth::user(), ['moderator', 'seller']))
                                            <a href="{{route('home.trades.complete', ['token' => $trade->token])}}" data-swal="confirm-ajax"
                                               ...
                                               data-text="{{__("The coin held on escrow will be released to buyer, as well as our standard fee which applies. This cannot be undone!")}}">
                                ...
                                <div class="chat chat-left mb-2">
                                    ...
                                    <div class="chat-body">
                                        <div class="chat-content" v-for="message in chat.content">
                                            <p>
                                                <small v-text="chat.user.name + ':'"></small> <br/>
    ...
    @push('mixins')
        <script type="text/javascript">
            window._vueData = {!! json_encode([
                ...
                'text_endpoint' => route('home.trades.send-message', ['token' => $trade->token]),
                'media_endpoint' => route('home.trades.upload-media', ['token' => $trade->token]),
...
@push('mixins')
    <script src="{{asset('js/pages/home/trades/mixin.js')}}" type="text/javascript"></script>
@endpush

@push('scripts')
    <script src="{{asset('js/pages/home/trades/script.js')}}" type="text/javascript"></script>
@endpush
